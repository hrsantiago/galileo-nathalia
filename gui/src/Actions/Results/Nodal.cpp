...
namespace gui
{
	...
	{
		//constructors
		Nodal::Nodal(fea::models::Model* model, const Results* results, QWidget* parent) : QDialog(parent), 
			m_play(false), m_ui(new Ui::Nodal), m_model(model), m_results(results)
		{
			//setup
			m_ui->setupUi(this);
			//data
			...
			const double**** vn = m_results->canvas()->node();
			...
			{
				if(i & m_results->canvas()->dofs())
				{
					m_ui->combo_dof->addItem(fea::mesh::nodes::Node::dof_name(fea::mesh::nodes::dof(i)));
				}
			}
			//set combo state
			const char* state[] = {"State", "Velocity", "Acceleration"};
			for(unsigned i = 0; i < 3; i++)
			{
				if(1 << i & m_model->analysis()->solver()->state_set())
				{
					m_ui->combo_type->addItem(state[i]);
				}
			}
			//connect
			...
			QObject::connect(m_ui->combo_dof, SIGNAL(currentIndexChanged(int)), this, SLOT(slot_combo(void)));
			QObject::connect(m_ui->combo_type, SIGNAL(currentIndexChanged(int)), this, SLOT(slot_type(void)));
			//maximize
			showMaximized();
		}
		...
		void Nodal::slot_push(void)
		{
			...
			for(unsigned i = 0; i < 2; i++)
			{
				...
				{
					const unsigned d = m_ui->combo_dof->currentIndex();
					const unsigned t = m_ui->combo_type->currentIndex();
					m_ui->slider->setValue(m_results->canvas()->limit_dof(t, d).m_step[i]);
				}
			}
		}
		void Nodal::slot_type(void)
		{
			m_ui->canvas->type(m_ui->combo_type->currentIndex());
		}
		...
		void Nodal::slot_print(void)
		{
			...
			const QString file = path + "/Nodal " + QString::asprintf("%04d", step) + ".png";
			//print
			m_ui->canvas->grabFramebuffer().save(file);
			QMessageBox::information(nullptr, "Nodal", "Nodal image saved!", QMessageBox::Ok);
		}
		void Nodal::slot_combo(void)
		{
			m_ui->canvas->dof(m_ui->combo_dof->currentIndex());
		}
		...
		void Nodal::slot_limits(void)
		{
			...
			//solver
			const unsigned ns = m_results->path()->steps();
			const unsigned dof = m_ui->combo_dof->currentIndex();
			const unsigned type = m_ui->combo_type->currentIndex();
			...
			//dof
			ui.dof->setTitle("DOF: " + m_ui->combo_dof->currentText());
			ui.label_step_min->setText(QString::asprintf("%04d", m_results->canvas()->limit_dof(type, dof).m_step[0]));
			ui.label_step_max->setText(QString::asprintf("%04d", m_results->canvas()->limit_dof(type, dof).m_step[1]));
			ui.label_node_min->setText(QString::asprintf("%05d", m_results->canvas()->limit_dof(type, dof).m_node[0]));
			ui.label_node_max->setText(QString::asprintf("%05d", m_results->canvas()->limit_dof(type, dof).m_node[1]));
			ui.label_value_min->setText(QString::asprintf("%+.2e", m_results->canvas()->limit_dof(type, dof).m_value[0]));
			ui.label_value_max->setText(QString::asprintf("%+.2e", m_results->canvas()->limit_dof(type, dof).m_value[1]));
			//show
			dialog.exec();
		}
