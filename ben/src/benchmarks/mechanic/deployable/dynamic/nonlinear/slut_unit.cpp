...
static void create_loads(fea::models::Model* model)
{
	...
	{
		const double t = (2 * i + 1) * M_PI / nd;
		model->boundary()->load_case(0)->add_load_node(22 * i + 10, fea::mesh::nodes::dof::translation_x, (open ? -Pr : +Pr) * cos(t));
		model->boundary()->load_case(0)->add_load_node(22 * i + 10, fea::mesh::nodes::dof::translation_y, (open ? -Pr : +Pr) * sin(t));
		model->boundary()->load_case(0)->add_load_node(22 * i + 17, fea::mesh::nodes::dof::translation_x, (open ? -Pr : +Pr) * cos(t));
		model->boundary()->load_case(0)->add_load_node(22 * i + 17, fea::mesh::nodes::dof::translation_y, (open ? -Pr : +Pr) * sin(t));
	}
}
