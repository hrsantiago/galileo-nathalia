...
void course::static_linear::frame(void)
{
	//parameters
	const unsigned n = 5;
	const unsigned m = 10;
	...
	//model
	fea::models::Model model("frame", "course/static_linear");
	...
	for(unsigned i = 1; i <= m; i++)
	{
		model.boundary()->load_case(0)->add_load_node(i, fea::mesh::nodes::dof::translation_x, 1e6);
	}
